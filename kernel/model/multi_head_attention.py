...
class MultiheadAttention(Module):
    ...
    def forward(self,
        ...
        #         for c in range(q.size(2)):
        #             if (q[i][j][c] < 0):
        #                 q[i][j][c] = torch.exp(q[i][j][c])-1;
        #             q[i][j][c] = q[i][j][c] + 1;
        # i=0;
        # j=0;
        # c=0;
        ...
        #         for c in range(k.size(2)):
        #             if (k[i][j][c] < 0):
        #                 k[i][j][c] = torch.exp(k[i][j][c])-1;
        #             k[i][j][c] = k[i][j][c]+1;
        m = nn.ELU()
        q = m(q) + 1
        k = m(k) + 1
        ...
        # Sum_attn_output_weights=torch.sum(attn_output_weights,1)
        # attn_output_weights=attn_output_weights / Sum_attn_output_weights
        # attn_output_weights = attn_output_weights.transpose(1,2)
...
#        assert list(attn_output_weights.size()) == [batch_size * num_heads, tgt_len, src_len]
        ...
        attn_output = torch.bmm(q, attn_output_weights)
